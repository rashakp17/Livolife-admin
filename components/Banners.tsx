  ...
  return (
    <div>
      <style>{`
        ...
        .btn-primary { background: #2F4E86; color: #fff; border: none; border-radius: 8px; padding: 10px 20px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s ease; }
        .btn-primary:hover { background: #24406F; }
        .btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
        .btn-ghost { background: transparent; color: #7E93B4; border: 1px solid #2A3C5F; border-radius: 8px; padding: 6px 12px; font-size: 12px; cursor: pointer; transition: all 0.2s ease; }
        ...
        .toggle-slider { position: absolute; inset: 0; background: #2A3C5F; border-radius: 22px; cursor: pointer; transition: 0.2s; }
        .toggle-slider:before { content: ''; position: absolute; height: 16px; width: 16px; left: 3px; bottom: 3px; background: #7E93B4; border-radius: 50%; transition: 0.2s; }
        input:checked + .toggle-slider { background: #2F4E86; }
        input:checked + .toggle-slider:before { transform: translateX(16px); background: #fff; }
      
        // .upload-box:hover { border-color: #2F4E86; }
        ...
        .banner-preview-label { font-size: 10px; font-weight: 600; color: #5C7095; text-transform: uppercase; letter-spacing: 0.05em; }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12, width: "100%" }}>
        <div>
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(20px, 5vw, 26px)", fontWeight: 800, color: "#E8EFF8", margin: 0 }}>Banners</h1>
          <p style={{ fontSize: 13, color: "#5C7095", margin: "4px 0 0 0" }}>{banners.filter(b => b.isActive).length} active</p>
        </div>
                ...
                <label className="upload-box" style={{ cursor: "pointer", padding: "20px 16px" }}>
                  {desktopImage ? (
                    <div style={{ position: "relative", width: "100%", height: "150px" }}>
                      ...
                      />
                      <div style={{ position: "absolute", top: 8, right: 8, background: "#2F4E86", color: "#fff", padding: "4px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600 }}>
                        Selected
                      </div>
                    </div>
                  ) : (
                    <div style={{ padding: "30px 0" }}>
                      <div style={{ fontSize: 13, color: "#7E93B4", fontWeight: 500 }}>📁 Click to upload desktop image</div>
                      <div style={{ fontSize: 11, color: "#5C7095", marginTop: 6 }}>Recommended: 1920x400px</div>
                    </div>
                  )}
              ...
              <Field label="Mobile Image (Required)">
                <label className="upload-box" style={{ cursor: "pointer", padding: "20px 16px" }}>
                  {mobileImage ? (
                    ...
                    <div style={{ padding: "30px 0" }}>
                      <div style={{ fontSize: 13, color: "#7E93B4", fontWeight: 500 }}>📱 Click to upload mobile image</div>
                      <div style={{ fontSize: 11, color: "#5C7095", marginTop: 6 }}>Recommended: 540x600px</div>
                    </div>
                  )}
...
function Field({ label, children }: FieldProps) {
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 600, color: "#7E93B4", marginBottom: 6 }}>{label}</div>
      {children}
    </div>
  );
}
